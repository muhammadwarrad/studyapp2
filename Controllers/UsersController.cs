...
public class UsersController : ControllerBase
{
    ...
    [HttpDelete("{userId}/flashcardSet")]
    public IActionResult DeleteFlashcardSetFromUser(int id)
    {
        var response = new Response();
        var flashcardSetToDelete = _context.Set<FlashcardSet>().FirstOrDefault(fs => fs.Id == id);
    
        var user =_context.Set<User>().FirstOrDefault(f => f.Id == id);
        ...
        if (user != null)
        {
            if (flashcardSetToDelete != null) _context.Set<FlashcardSet>().Remove(flashcardSetToDelete);
        }
